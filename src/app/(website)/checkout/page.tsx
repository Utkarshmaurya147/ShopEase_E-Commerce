...
export default function CheckoutPage() {
  ...
  // 1. SAFE SUBTOTAL CALCULATION (Prevents build error)
  ...
  const handleRazorpayPayment = async (shippingAddress: string) => {
    ...
    try {
      // 2. BACKEND CALL: Razorpay expects amount in the smallest currency unit (paise)
      // Usually, the backend handles the *100, but ensure your payload matches your API
      ...
      const options = {
        ...
        amount: razorRes.order.amount, // This should be (total * 100) from backend
        currency: "INR",
        name: "ShopEase",
            ...
            const verifyData = {
              ...
              orderInfo: {
                ...
                items: cartItems.map((item) => ({
                  productId: item.id,
                  quantity: item.quantity,
                  // 3. SAFE PRICE PARSING for verification payload
                  price:
  ...
  const handleAfterOrderSuccess = async (
    ...
    if (updateProfileAddress && user) { // Added 'user' check here
      try {
        ...
        setUser({
          ...
          address: shippingAddress,
          phone: formData.shipping_phone,
          name: `${formData.firstName} ${formData.lastName}` // Keep name synced too
        });
      } catch (err) {
        ...
        <div className="relative max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-2 font-medium">
              Finalize your order details securely.
            </p>
          </div>
          <Link
            href="/cart"
            className="flex items-center gap-2 text-sm font-black text-blue-400 hover:text-white transition-colors"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="pt-8 border-t border-gray-100">
                <h3 className="text-xl font-black text-gray-900 mb-6 tracking-tight flex items-center gap-2">
                  <CreditCardIcon className="h-6 w-6 text-blue-600" /> Payment
                  Method
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <span
                      className={`font-black uppercase tracking-widest text-xs ${paymentMethod === "razorpay" ? "text-blue-600" : "text-gray-400"}`}
                    >
                      Online Payment
                    </span>
                  </button>
                  ...
                  >
                    <span
                      className={`font-black uppercase tracking-widest text-xs ${paymentMethod === "cod" ? "text-blue-600" : "text-gray-400"}`}
                    >
                      Cash on Delivery
                    </span>
                  </button>
          ...
          <div className="lg:w-[450px] sticky top-32">
            <div className="bg-gray-50/50 backdrop-blur-xl border border-gray-100 rounded-[40px] p-10 shadow-2xl shadow-gray-200/50">
              <h3 className="text-2xl font-black text-gray-900 mb-8 tracking-tighter">
                Summary
              </h3>
                    ...
                    <div key={item.id} className="flex gap-5 group">
                      <div className="relative w-20 h-20 flex-shrink-0">
                        <img
                          ...
                          className="w-full h-full bg-white rounded-[20px] object-cover border border-gray-100 shadow-sm"
                          alt={item.name}
                        />
                        <span className="absolute -top-2 -right-2 bg-blue-600 text-white text-[10px] font-black w-6 h-6 flex items-center justify-center rounded-full border-2 border-white">
                          {item.quantity}
                        </span>
                      </div>
                      <div className="flex-1 flex flex-col justify-center">
                        <h4 className="text-sm font-black text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1">
                          {item.name}
                        </h4>
                        {/* 5. FIX: Used numeric itemPrice */}
                        <p className="text-sm font-black text-blue-600 mt-1">
                          ${((itemPrice || 0) * item.quantity).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  );
              ...
              <div className="mt-8 flex items-center justify-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                <ShieldCheckIcon className="h-4 w-4 text-green-500" /> Secure
                SSL Encryption
              </div>
            </div>
          </div>
