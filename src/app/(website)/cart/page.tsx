...
export default function CartPage() {
  ...
  // 1. SAFE SUBTOTAL CALCULATION
  const subtotal = cartItems.reduce((acc, item) => {
    // Force parse just in case it's a string from the DB
        ...
        {cartItems.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-8 space-y-8">
              {cartItems.map((item) => {
                // 2. SAFE ITEM PRICE FOR RENDER
                const itemPrice = typeof item.price === 'string' ? parseFloat(item.price) : item.price;
                const rowTotal = (itemPrice || 0) * item.quantity;
                    ...
                    <div className="mt-4 sm:mt-0 sm:ml-8 flex flex-1 flex-col justify-between">
                      <div>
                          <div className="flex justify-between items-start">
                              ...
                              {/* 3. FIX: Use rowTotal which is guaranteed to be a number */}
                              <p className="text-xl font-black text-gray-900">${rowTotal.toLocaleString()}</p>
                          </div>
                      </div>
