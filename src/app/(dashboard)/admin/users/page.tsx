...
export default function AdminUsersPage() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!window.confirm(`Are you sure you want to remove ${name}? This will archive the account.`)) return;
    try {
  ...
  const toggleRole = async (user: any) => {
    ...
    try {
      ...
      setUsers(prev => prev.map((u: any) => u.id === user.id ? { ...u, role: newRole } : u));
    } catch (err) {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        ...
        <div className="bg-blue-600 px-6 py-3 rounded-2xl text-white shadow-xl shadow-blue-900/20">
            <span className="text-[10px] font-black uppercase tracking-widest block opacity-80">Total Users</span>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {filteredUsers.length === 0 && !loading && (
          <div className="p-20 text-center text-slate-400 font-medium">No customers found matching that search.</div>
        )}
      </div>
    </div>
  );
